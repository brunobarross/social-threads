<template>
  <article class="feed-card">
    ...
    <div class="icons mt-2">
      <button @click="handleClickPostLike(thread?.id)">
        <svg
          clip-rule="evenodd"
          fill-rule="evenodd"
      ...
      <button @click="handleClickOpenPopUp">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
          height="24"
...
const timePassed = computed(() => {
  return moment().startOf("day").fromNow(props.thread.timestamp);
});
...
const handleClickOpenPopUp = () => {
  setInterectedThread(props.thread.id);
  setIsOpenPopUp(true);
};
</script>